...
<html>
  ...
  <body>
    ...
    <div id="right">
		...
		<div align="center">
			<a href="<%=request.getContextPath() %>/servlet/ScoreSelectsServlet?curPage=1&operation=select" target="content" class="btn btn-primary">首页</a>
			<a href="<%=request.getContextPath() %>/servlet/ScoreSelectsServlet?curPage=<%=pages-1 %>&operation=select" target="content" class="btn btn-primary">上一页</a>
			当前是:<%=pages %>页&nbsp;总共:<%=maxPage %>页
			<a href="<%=request.getContextPath() %>/servlet/ScoreSelectsServlet?curPage=<%=pages+1 %>&operation=select" target="content" class="btn btn-primary">下一页</a>
			<a href="<%=request.getContextPath() %>/servlet/ScoreSelectsServlet?curPage=<%=maxPage %>&operation=select" target="content" class="btn btn-primary">末页</a>
		</div>
    </div>
  </body>
</html>
